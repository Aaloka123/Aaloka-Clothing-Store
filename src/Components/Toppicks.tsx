import React from "react";
import Timg1 from "../assets/Timg1.svg";
import Timg2 from "../assets/Timg2.svg";
import Timg3 from "../assets/Timg3.svg";
import Timg4 from "../assets/Timg4.svg";
import { ChevronDown } from "lucide-react";
import { Link } from "react-router-dom";

const Toppicks = () => {
  const toppickItems = [
    { id: 1, img: Timg1, name: "Marron Long Coat", price: "RS. 8000.0" },
    { id: 2, img: Timg2, name: "Marron Long Coat", price: "RS. 8000.0" },
    { id: 3, img: Timg3, name: "Marron Long Coat", price: "RS. 8000.0" },
    { id: 4, img: Timg4, name: "White Tshirt", price: "RS. 8000.0" },
  ];

  return (
    <div className="bg-secondaray px-4 sm:px-8 lg:px-20 py-12">
      <div className="h-1 bg-primary w-full" />

      <p className="font-blastula text-primary text-2xl sm:text-3xl md:text-4xl lg:text-[40px] text-center mt-6">
        Top Picks
      </p>

      <div className="flex justify-end mt-4">
        <button className="flex items-center font-normal text-primary text-sm sm:text-base">
          Trending <ChevronDown className="ml-1" />
        </button>
      </div>

      {/* Responsive grid like Featured Collection */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-10 mt-6">
        {toppickItems.map(({ id, img, name, price }) => (
          <div
            key={id}
            className="text-center group cursor-pointer transition-all duration-300"
          >
            <Link to={`/Toppickspage`}>
              <div className="overflow-hidden rounded-lg">
                <img
                  src={img}
                  alt={name}
                  className="w-full h-auto max-h-96 transition-transform duration-300 group-hover:scale-105 object-contain"
                />
              </div>
            </Link>

            <p className="mt-2 font-semibold text-lg text-gray-800 transition-colors duration-300 group-hover:text-primary">
              {name}
            </p>
            <p className="text-gray-500 transition-colors duration-300 group-hover:text-primary">
              {price}
            </p>
          </div>
        ))}
      </div>

      <div className="flex justify-center mt-10">
        <Link to="/ToppicksPage">
          <button className="bg-primary text-white h-12 sm:h-14 text-sm sm:text-base px-6 sm:px-10 py-3 mt-6 mb-8 transition-all duration-300 hover:bg-primary/80 hover:shadow-lg hover:scale-105 hover:underline">
            SHOP NOW
          </button>
        </Link>
      </div>
    </div>
  );
};

export default Toppicks;
